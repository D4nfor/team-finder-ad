...
def user_detail(request, pk):
    profile_user = get_object_or_404(User.objects.prefetch_related("skills", "owned_projects"), pk=pk)
...
def skills_search(request):
    ...
    skills = (
        Skill.objects.filter(name__istartswith=query).order_by("name")[:SKILLS_SUGGESTIONS_LIMIT]
        if query
...
def add_skill(request, pk):
    ...
    if skill is None:
        return JsonResponse({"error": "skill_id or name is required"}, status=HTTPStatus.BAD_REQUEST)
    before = profile_user.skills.filter(pk=skill.pk).exists()
    profile_user.skills.add(skill)
    return JsonResponse(
        {"id": skill.id, "name": skill.name, "skill_id": skill.id, "created": created, "added": not before}
    )
...
def remove_skill(request, pk, skill_id):
    ...
    if not profile_user.skills.filter(pk=skill.pk).exists():
        return JsonResponse({"error": "skill is not attached"}, status=HTTPStatus.NOT_FOUND)
